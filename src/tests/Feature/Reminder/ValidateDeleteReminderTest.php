...
class ValidateDeleteReminderTest extends TestCase
{
    use RefreshDatabase;

    /** @test */
    public function cannot_delete_reminder_record_because_resource_not_found(): void
    {
        Sanctum::actingAs(User::factory()->create(), ['access-api']);

        $response = $this->deleteJson('/api/reminders/1');
    ...
    /** @test */
    public function cannot_delete_reminder_record_because_its_not_belongs_to_current_user(): void
    {
        Sanctum::actingAs(User::factory()->create(), ['access-api']);
